...
*/
    if (typeof (keylen) !== 'number') { this.chunkSize = 2 * biHighIndex(this.m) } else { this.chunkSize = keylen / 8 }
...
*/
    if (typeof (pad) === 'string') {
        if (pad == RSAAPP.NoPadding) { padtype = 1 } else if (pad == RSAAPP.PKCS1Padding) { padtype = 2 } else { padtype = 0 }
    } else { padtype = 0 }
/*
...
*/
    ...
    if (padtype == 2) { j = sl - 1 } else { j = key.chunkSize - 1 }

    while (i < sl) {
        if (padtype) { a[j] = s.charCodeAt(i) } else { a[i] = s.charCodeAt(i) }
...
*/
  ...
  */
        if (key.radix == 16) { bi = biFromHex(blocks[i]) } else { bi = biFromString(blocks[i], key.radix) }
  /*
